...
$(document).ready(function(){
  	...
  	var table_companies = $('#table_chapitre').dataTable({
		
	"bStateSave": false,
	
	
    "ajax": "table/php/data_liste_chapitre.php?job=get_liste_chapitre",
...
// add new sidebar starts //	
  $(document).on('click', '#add_chapitre', function(e){
	 e.preventDefault();
	 	
			$('#titre_h4').text("Ajouter un nouveau chapitre");
      ...
      request.done(function(output){
        if (output.result == 'success'){
			...
			table_companies.api().ajax.reload(function(){
				Swal.fire({
				  title: "BRAVO !",
				  text: "Nouveau chapitre ajouté avec succés",
				  type: "success",
				  confirmButtonClass: 'btn btn-primary',
				  buttonsStyling: false,
				});
   				//toastr.success('Nouveau chapitre avec succés', 'succés', { positionClass: 'toast-bottom-full-width' , "progressBar": true , "closeButton": true }); 					
			}, true);
...
$(document).on('click', '#confirm-color', function(e){
		...
		Swal.fire({
		  ...
		  if (result.value) {
							e.preventDefault();
							var id_supp      = $("#confirm-color").data('id');
							var name      = $("#confirm-color").data('name');
							var request = $.ajax({
							url:          'table/php/data_liste_chapitre.php?job=del_chapitre&id=' + id_supp,
							cache:        false,
							dataType:     'json',
							contentType:  'application/json; charset=utf-8',
							type:         'get'
								...
								if (output.result == 'success'){
								  table_companies.api().ajax.reload(function(){
									  Swal.fire({
										  type: "success",
										  title: 'Supprimée!',
										  text: "Chapitre '" + name + "' effacé avec succès.",
  ...
  $(document).on('click', '#function_edit_chapitre', function(e){
		...
		var request = $.ajax({
		  url:          'table/php/data_liste_chapitre.php?job=get_chapitre_edit',
		  cache:        false,
		  data:         'id=' + id,
		  dataType:     'json',
		  contentType:  'application/json; charset=utf-8',
	  ...
	  $(document).on('submit', '#form_company.edit', function(e){
		...
		if (form_company.valid() == true){
		  ...
		  var form_data = $('#form_company').serialize();
		  var request   = $.ajax({
			url:          'table/php/data_liste_chapitre.php?job=chapitre_edit&id=' + id,
			cache:        true,
			data:         form_data,
			dataType:     'json',
			contentType:  'application/json; charset=utf-8',
