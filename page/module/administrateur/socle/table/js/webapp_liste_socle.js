...
$(document).ready(function(){
  	...
  	var table_companies = $('#table_socle').dataTable({
...
var form_company = $('#form_company');
  ...
  $(document).on('click', '#add_socle', function(e){
	 e.preventDefault();
	 	
			$('#titre_h4').text("Ajouter un nouveau socle");
      ...
      request.done(function(output){
        if (output.result == 'success'){
			...
			table_companies.api().ajax.reload(function(){
				Swal.fire({
				  title: "BRAVO !",
				  text: "Nouveau socle ajouté avec succés",
				  type: "success",
				  confirmButtonClass: 'btn btn-primary',
				  buttonsStyling: false,
				});
   				//toastr.success('Nouveau socle avec succés', 'succés', { positionClass: 'toast-bottom-full-width' , "progressBar": true , "closeButton": true }); 					
			}, true);
...
$(document).on('click', '#confirm-color', function(e){
		...
		Swal.fire({
		  ...
		  if (result.value) {
							...
							var request = $.ajax({
							url:          'table/php/data_liste_socle.php?job=del_socle&id=' + id,
							cache:        false,
							dataType:     'json',
							contentType:  'application/json; charset=utf-8',
							type:         'get'
								...
								if (output.result == 'success'){
								  table_companies.api().ajax.reload(function(){
									  Swal.fire({
										  type: "success",
										  title: 'Supprimée!',
										  text: "Socle '" + name + "' effacé avec succès.",
   ...
   $(document).on('click', '#function_edit_socle', function(e){
		e.preventDefault();
		var id      = $(this).data('id');
		var name      = $(this).data('name');
		
		var request = $.ajax({
		  url:          'table/php/data_liste_socle.php?job=get_socle_edit',
		  cache:        false,
		  data:         'id=' + id,
		  dataType:     'json',
		  contentType:  'application/json; charset=utf-8',
		...
		request.done(function(output){
		  if (output.result == 'success'){ 
				...
				$('#titre_h4').text("Modifier le socle : " + name);
				...
				$('#form_company #nom_socle').val(output.data[0].nom_socle);
	  ...
	  $(document).on('submit', '#form_company.edit', function(e){
		...
		if (form_company.valid() == true){
		  ...
		  var form_data = $('#form_company').serialize();
		  var request   = $.ajax({
			url:          'table/php/data_liste_socle.php?job=socle_edit&id=' + id,
			cache:        true,
			data:         form_data,
			dataType:     'json',
			contentType:  'application/json; charset=utf-8',
		  ...
		  request.done(function(output){
			if (output.result == 'success'){
				...
				table_companies.api().ajax.reload(function(){
					Swal.fire({
					  title: "BRAVO !",
					  text: "socle modifié avec succés",
					  type: "success",
					  confirmButtonClass: 'btn btn-primary',
					  buttonsStyling: false,
					});
