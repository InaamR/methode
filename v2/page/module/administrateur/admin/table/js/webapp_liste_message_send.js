 ...
 $(function () {
	...
	if (dt_basic_table.length) {
	  var dt_basic = dt_basic_table.DataTable({
		ajax: 'table/php/data_liste_message_send.php?job=get_liste_message_send',
		columns: [    
		  { data: 'responsive_id' },
		  { data: 'id' },
		  { data: 'id' }, 
		...
		language: {
		  ...
		  search: "Recherche :",
		  zeroRecords: "Vous n\'avez pas envoy&eacute; de message",
		  infoEmpty: "Vous n\'avez pas envoy&eacute; de message",
		  infoFiltered: "(filtré depuis _MAX_ total des enregistrements)",
		  loadingRecords: "Veuillez patienter - chargement ..."
		}
	  });
	  $('div.head-label').html('<h6 class="mb-0">Liste des messages envoyés</h6>');
	...
	$(document).on('submit', '#form_message', function(e){
		e.preventDefault();
		  ...
		  var request   = $.ajax({
			url:          'table/php/data_liste_message_send.php?job=add_message',
			data:         form_data,
			type:         'post',
			async: false,
	...
	$(document).on('submit', '#form_message_all', function(e){
		e.preventDefault();
		  ...
		  var request   = $.ajax({
			url:          'table/php/data_liste_message_send.php?job=add_message_all',
			data:         form_data,
			type:         'post',
			async: false,
			beforeSend: onBeforeSend,
